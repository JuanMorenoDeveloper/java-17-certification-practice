...
class Ch1BuildingBlocksTest {
  ...
  private static final Predicate<String> REGEX_LEADING_WHITESPACE =
      Pattern.compile("\\s+.*").asMatchPredicate();
  private static final Predicate<String> REGEX_TRAILING_WHITESPACE =
      Pattern.compile(".*\\s+").asMatchPredicate();
  ...
  void verifyTextBlock() {
    ...
    assertThat(textBlock.split(LINEBREAK_MATCHER))
        .anyMatch(REGEX_TRAILING_WHITESPACE)
        .noneMatch(REGEX_LEADING_WHITESPACE);

    assertThat(textBlock2).hasLineCount(3);
    assertThat(textBlock2.split(LINEBREAK_MATCHER))
        .anyMatch(REGEX_LEADING_WHITESPACE)
